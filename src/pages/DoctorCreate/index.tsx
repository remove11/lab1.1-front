...
const DoctorCreate: React.FC = () => {
  ...
  const [degreeId, setDeggreeId] = useState("")
  ...
  const onLogin = async () => {
    const data:IDoctorCreate = {
      ...
      degreeId: degreeId,
      type: "patient",
      getCalenderId: "NoCalId"
    }

    createDoctor(data).then(res => {
      const authToken = Cookies.get("Authorization")
      if(res && authToken) navigate("/app/doctors")
    })
  }
  ...
  <main className='flex justify-center items-center min-h-screen'>
    <section className='w-screen'>
      <div className='border w-[500px] px-16 py-9 rounded-3xl m-auto'>
        ...
        <div className='flex w-full bg-white py-3 px-4 rounded-full shadow-lg items-center my-7'>
          <FontAwesomeIcon icon={faCircleInfo} color='#6E6E6E'/>
          <input className='border-transparent focus:border-transparent focus:ring-0 focus:outline-none ml-3' placeholder='Degree id' onChange={(e:ChangeEvent<HTMLInputElement>) => setDeggreeId(e.target.value)}/>
        </div>
